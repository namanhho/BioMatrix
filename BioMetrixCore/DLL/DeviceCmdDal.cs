...
insert into DeviceCmds(
       ...
       @DevSN,@Content,@CommitTime,@TransTime,@ResponseTime,@ReturnValue
        ...
        /// <returns></returns>
        public string Send(string DevSN, bool isInbio)
        {
            string sql = string.Format($@"
select  ID, DevSN, Content, CommitTime, TransTime, ResponseTime, ReturnValue 
  from DeviceCmds
 where DevSN=@DevSN and (ReturnValue is null or length(ReturnValue)=0) limit {(isInbio ? 1 : 100)}
");
        ...
        public DataTable GetByTime(DateTime startTime, DateTime endTime, string devSN)
        {
            ...
            string sql = string.Format(@"
select *,substr(Content,0,50000) as subContent from DeviceCmds
...
", whereDevSN);
        ...
        {
            string sql = string.Format(@"
select *,substr(Content,0,50000) as subContent   from DeviceCmds order by CommitTime desc
");
